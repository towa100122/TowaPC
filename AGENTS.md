...
- v43の表示変更はコミット `c48cd25 Keep About card light and add home links` まで `main` へ公開済み。
- ホーム以外の全ページに「← ホームに戻る」を1件ずつ追加し、ホームには表示しない構成にした。
- ダークテーマでもAboutの「TowaPCについて」カードは白背景・黒文字、カード内リンクは薄いグレー背景・黒文字で表示する。
- ContactのXロゴはライト・ダークとも黒で固定。Join最上部のカードはテーマと反転する。
- Information一覧の外側は透明で、角の後ろに四角い背景が見えない構成にした。
- 390px幅で横はみ出しがなく、ホーム以外の10経路にホームリンクが各1件、ホームには0件であることを確認済み。
