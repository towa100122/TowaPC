...
- キャッシュ番号を変更するときは `templates/page.html` のCSS・JavaScriptと、`app-v2.js` の `site-data.js` importを同じ番号にする。現在は `v46`。
...
- v46の404メッセージ変更はコミット `8c6b2d8 Add the lost page message` で確定。
- v45の404・えにいゲーム変更はコミット `81d72e4 Turn the 404 secret into an enii game` で確定。
- 404上部の `Page not found` と「ページが見つかりませんでした」を削除し、本文を `404 / Not found / お探しのページは迷子かもしれません。` の構成へ変更。ブラウザーのページ名も `TowaPC — Not found` にした。
...
- 1280×720のダークテーマで404の3行がカード中央と一致し、新しい案内文を含めてブラウザーエラーがないことを確認済み。
