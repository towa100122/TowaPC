...
- キャッシュ番号を変更するときは `templates/page.html` のCSS・JavaScriptと、`app-v2.js` の `site-data.js` importを同じ番号にする。現在は `v49`。
...
- 404カード右下には、34px四方・不透明度0.32の紫とシアンの小さな発光点がある。人が注意して見れば気付く程度の隠しボタンで、押すと「AIセンスで作った全く謎の謎の\"えにい\"ゲーム」が開く。
- えにいゲームは4種類。逃げる文字を押す「えにい捕獲」、数字を合わせた404を即廃棄する「404工場」、存在直前に初期化される「ページ育成」、99%で再試行する「虚無ロード」。全モードで失った時間を共有し、クリア条件はない。ライト・ダーク両テーマに対応する。
- 404工場で `334` を製造すると、1.8秒間の専用警報を発動する。中央へ `33-4` の札を出し、工場を揺らして色を変え、`なんでや！阪神関係ないやろ！` と `阪神関係 / ないやろ` を表示する。同じ数字で何度でも再発動できる。
- えにいゲームの見た目は、紫・シアン・ピンクの発光グラデーション、ガラス面、英字ラベル、ニューラル風の方眼、光沢アニメーションを重ね、意図的に過剰な生成AI風デザインにする。閉じるボタンはタイトル幅の影響を受けない38×38pxの真円。
...
- v49の334警報はコミット `7ea300e Add a 33-4 factory alert` で確定。
- v48の複数ゲーム・発光入口変更はコミット `6b38b7c Add a collection of strange 404 games` で確定。
- v47のえにいゲーム外観変更はコミット `d6472d4 Give the enii game an excessive AI aesthetic` で確定。
...
- 右下の `✦` は廃止し、カード右下でごく弱く光る小さな隠しボタンへ変更した。
- ペイントゲームはコードとスタイルを含めて削除。タイトルどおり意味の分からない4種類のクリア不能ゲームへ置き換えた。
...
- 1280×720のライト・ダークと390×844のダークで新デザインを目視確認済み。ダイアログとページに縦横の不要なはみ出しはなく、閉じるボタンは全条件で38×38px・`border-radius: 50%`。
- 4モードの切替、404の製造と廃棄、育成の初期化、虚無ロードの99%再試行を操作確認済み。PC・390×844の全モードでダイアログの縦横はみ出しなし。
- ライトテーマのPCとダークテーマの390×844で334警報を確認済み。警報解除後の再発動、`33-4`表示、メッセージ、専用指標が正常で、縦横はみ出しとブラウザーエラーなし。
