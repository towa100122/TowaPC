...
- 通常カードとセクション見出しの登場は、下42px・0.965倍・8pxぼかし・透明から定位置・等倍・ぼかしなしへ0.68秒でフェードアップする。減速カーブは `cubic-bezier(0.16, 1, 0.3, 1)`、カード間の遅延は48ms。Appleのサイトに近い滑らかさを保ちつつ短めにする。
...
- PCと390×844のホームでカードの順次表示を確認済み。390×844では横はみ出しなし、ブラウザーエラーなし。
