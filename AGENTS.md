...
- 通常カードとセクション見出しの登場は、下42px・0.965倍・8pxぼかし・透明から定位置・等倍・ぼかしなしへ0.52秒でフェードアップする。減速カーブは `cubic-bezier(0.16, 1, 0.3, 1)`、カード間の遅延は42ms。Appleのサイトに近い滑らかさを保ちつつ短めにする。
...
- v52の通常カード登場アニメーションはコミット `01ad855 Make card reveals smoother and faster` で確定。
...
- PCと390×844のAboutカードで0.52秒のアニメーション、減速カーブ、途中の透明度・移動・拡大・ぼかしを計測確認済み。両幅で横はみ出しとブラウザーエラーなし。
