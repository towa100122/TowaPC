...
- Homeの「What’s TowaPC?」はカード化せず、ページ背景上の導入文として表示する。
...
- Aboutのロゴ・紹介文・3つの関連ボタンは固定の白カードへ入れず、ページ背景上のひと続きの領域として表示する。本文はテーマの文字色へ追従し、ボタンはライト／ダークの両方で十分なコントラストを保つ。
...
- Aboutはロゴ、本文、3つの関連ボタンをカード化しない紹介領域へ置き、その下へ履歴と名前の由来を表示する。
