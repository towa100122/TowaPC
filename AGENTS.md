...
- 外観設定では、明るさ（自動・Light・Dark）、差し色（ひまわり・藤・若葉・夕焼け）、カードの角、余白、UIの大きさ、ヘッダーのガラス効果も変更できる。
...
- PCと390×844で隠し入口、設定操作、保存、リセット、標準・ゆったり・静止の通常ページ反映を確認済み。390×844では横はみ出しなし、ブラウザーエラーなし。
